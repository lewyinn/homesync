...
export default function EditPostPage({ params }) {
    ...
    const [form, setForm] = useState({
        id, title: "", slug: "", tag: "", date: "", read: "", excerpt: "", content: "", cover: "",
    });
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                console.error(e);
                MySwal.fire("Error!", e.message || "Failed to load post data.", "error");
            } finally {
                setLoading(false);
            }
        })();
    ...
    const handleSubmit = async (e) => {
        ...
        const { slug: _omit, ...payload } = form; // slug diatur server jika title berubah
        ...
        try {
            ...
            MySwal.fire({ title: "Success!", text: "Post updated successfully!", icon: "success" })
                .then(() => router.push("/admin/blogs"));
        } catch (e) {
            console.error(e);
            MySwal.fire("Error!", e.message || "Failed to update post.", "error");
        } finally {
            setSaving(false);
        }
    };
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 lg:p-8 transition-colors">
                <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                    <div>
                        ...
                        <input id="title" name="title" value={form.title} onChange={handleChange} required className="w-full px-4 py-2 border rounded-lg" />
                    </div>

                    <div>
                        <label htmlFor="slug" className="block text-sm font-medium mb-1">Slug (auto)</label>
                        <input id="slug" name="slug" value={form.slug || toSlug(form.title)} readOnly className="w-full px-4 py-2 border rounded-lg bg-gray-100 text-gray-500" />
                    ...
                    <div>
                        ...
                        <input id="date" name="date" value={form.date} onChange={handleChange} className="w-full px-4 py-2 border rounded-lg" />
                    </div>

                    <div>
                        <label htmlFor="read" className="block text-sm font-medium mb-1">Read</label>
                        <input id="read" name="read" value={form.read} onChange={handleChange} className="w-full px-4 py-2 border rounded-lg" />
                    </div>
                    ...
                    <div className="md:col-span-2 flex justify-end mt-4">
                        <button type="submit" disabled={saving || uploading} className="inline-flex justify-center py-2 px-6 rounded-md text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50">
                            {saving ? "Saving..." : "Save Changes"} <FaCheckCircle className="ml-2 w-5 h-5" />
                        </button>
                    </div>
                </form>
            </div>
