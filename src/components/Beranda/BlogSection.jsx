...
export default function BlogSection() {
    ...
    const blogsToShow = useMemo(() => items.slice(0, 3), [items]);
    ...
    return (
        <section id="blog" className="py-14 md:py-20">
            <div className="container mx-auto px-2 md:px-8">
                <div className="mb-8 md:mb-10 flex items-end justify-between gap-4">
                    <div>
                        <h2 className="text-2xl md:text-3xl font-extrabold text-black">
                            {headBlogs.title.split(" ")[0]}{" "}
                            <span className="text-blue-700">
                                {headBlogs.title.replace(headBlogs.title.split(" ")[0] + " ", "")}
                            </span>
                        </h2>
                        <p className="mt-2 text-black/70">{headBlogs.desc}</p>
                    </div>
                    <Link
                        href="/blogs"
                ...
                {loading ? (
                    <div className="text-black/60">Loading articles…</div>
                ) : blogsToShow.length === 0 ? (
                    <div className="text-black/60">No articles yet.</div>
                ) : (
                        ...
                        {blogsToShow.map((p) => (
                            ...
                            >
                                ...
                                <div className="p-5">
                                    ...
                                    <h3 className="text-black font-semibold text-lg leading-snug">{p.title}</h3>
                                    <p className="mt-2 text-black/75 text-sm">{p.excerpt}</p>
                                    <Link href={"/blogs/" + p.slug} className="mt-4 inline-flex items-center gap-2 text-blue-600 hover:underline">
                                        Read more →
                                    </Link>
                                </div>
                            </article>
