    ...
    async function onDelete() {
        ...
        if (result.isConfirmed) {
            try {
                ...
                console.error(e);
                MySwal.fire("Error!", "Failed to delete product.", "error");
            } finally {
                setLoading(false);
            }
        }
                    ...
                    <div className="hidden lg:block overflow-x-auto">
                        <table className="w-full">
                            ...
                            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                {loading ? (
                                    <tr><td className="px-6 py-6 text-sm text-gray-500 dark:text-gray-400" colSpan={7}>Loading...</td></tr>
                                ) : paginatedData.length === 0 ? (
                                    <tr><td className="px-6 py-6 text-sm text-gray-500 dark:text-gray-400" colSpan={7}>No data</td></tr>
                                ) : paginatedData.map((p) => (
                                    <tr key={p.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                                        ...
                                        <td className="px-6 py-4 text-sm text-gray-900 dark:text-white">{p.type}</td>
                                        <td className="px-6 py-4 text-sm text-gray-900 dark:text-white">{p.badge}</td>
                                        <td className="px-6 py-4 text-sm text-gray-900 dark:text-white">{p.year}</td>
                                        <td className="px-6 py-4 text-sm text-gray-900 dark:text-white">{p.location}</td>
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex items-center space-x-2 justify-end">
                                                <Link href={`/admin/products/${p.id}/edit`} className="text-blue-600 hover:text-blue-700 p-1" title="View">
                                                    <FaEye className="w-4 h-4" />
                                                </Link>
                                                <Link href={`/admin/products/${p.id}/edit`} className="text-green-600 hover:text-green-700 p-1" title="Edit">
                            ...
                            <div key={p.id} className="p-4 border-b border-gray-200 dark:border-gray-700 last:border-b-0">
                                ...
                                <div className="grid grid-cols-2 gap-4 mb-3">
                                    <div>
                                        ...
                                        <p className="text-sm text-gray-900 dark:text-white">{p.type || '-'}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Badge</p>
                                        <p className="text-sm text-gray-900 dark:text-white">{p.badge || '-'}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Year</p>
                                        <p className="text-sm text-gray-900 dark:text-white">{p.year || '-'}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Location</p>
                                        <p className="text-sm text-gray-900 dark:text-white">{p.location || '-'}</p>
                                    </div>
                                </div>

                                <div className="flex items-center space-x-3">
                                    <Link href={`/admin/products/${p.id}/edit`} className="flex items-center space-x-1 text-blue-600 hover:text-blue-700 text-sm">
                                        <FaEye className="w-4 h-4" />
                                        <span>View</span>
                                    </Link>
                                    <Link href={`/admin/products/${p.id}/edit`} className="flex items-center space-x-1 text-green-600 hover:text-green-700 text-sm">
                                        <FaEdit className="w-4 h-4" />
                                        <span>Edit</span>
                                    </Link>
                                    <button
