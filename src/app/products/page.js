...
export default function ProductsPage() {
    ...
    // fetch dari API (Blob/FS fallback sudah di-handle di route)
    useEffect(() => {
        (async () => {
            try {
                setLoading(true);
    ...
    // tipe dinamis dari data
    const types = useMemo(() => {
        const set = new Set(items.map((p) => p.type).filter(Boolean));
                    ...
                    {!loading && filtered.length === 0 ? (
                        <div className="rounded-2xl bg-black/5 backdrop-blur-md border border-black/10 p-8 text-center">
                            <p className="text-black/80">
                                Tidak ada properti yang cocok dengan pencarianmu. Coba ubah kata kunci atau filter tipe.
                            </p>
                        </div>
                    ) : (
                        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
                            {(loading ? Array.from({ length: 6 }) : filtered).map((p, idx) => (
                                ...
                                >
                                    {loading ? (
                                        <div className="h-48 animate-pulse bg-black/10" />
                                    ) : (
                                        <>
                                            {/* Cover */}
                                            <Link href={`/products/${p.slug}`} className="relative block h-44 md:h-48 overflow-hidden">
                                                ...
                                                />
                                                {p.badge && (
                                                    <span className="absolute top-3 left-3 px-3 py-1 rounded-full bg-blue-600 text-neutral-200 text-xs font-semibold">
                                                        {p.badge}
                                                    </span>
                                                )}
                                            </Link>
                                            ...
                                            <div className="p-5 flex flex-col flex-1">
                                                ...
                                                <div className="mt-auto pt-4 flex items-center justify-between">
                                                    <Link
                                                        ...
                                                        className="px-4 py-2 rounded-lg border border-blue-600 text-blue-400 hover:bg-blue-800 hover:text-neutral-200 transition text-sm font-semibold"
                                                    >
                                                        See More →
                                                    </Link>
